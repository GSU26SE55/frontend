...
Shape thực tế khi `data` là danh sách phân trang (`PaginationResponse<AccountDto>`, `PaginationResponse<RoleDto>`...):
...
| Field | Type | Mô tả |
|---|---|---|
| `items` | `T[]` | Danh sách item trong trang hiện tại |
| `totalItems` | `int` | Tổng số item trong toàn bộ result set |
| `pageNumber` | `int` | Trang hiện tại (bắt đầu từ 1) |
| `pageSize` | `int` | Số item tối đa mỗi trang |
...
Shape đầy đủ — dùng cho `GET /api/auth/me`, `GET /api/admin/accounts`, `GET /api/admin/accounts/{id}`:

| Field | Type | Nullable | Mô tả |
|---|---|---|---|
| `id` | `Guid` | Không | ID tài khoản |
| `email` | `string` | Không | Email đăng nhập |
| `phoneNumber` | `string?` | Null nếu chưa có | Số điện thoại |
| `fullName` | `string` | Không | Họ và tên |
| `avatarUrl` | `string?` | Null — legacy field, deprecated | URL avatar trực tiếp (không dùng để render, dùng `displayAvatarUrl`) |POST /api/admin/accounts/invit
| `dateOfBirth` | `DateTime?` | Null nếu chưa có | Ngày sinh |
| `address` | `string?` | Null nếu chưa có | Địa chỉ |
| `emailConfirmed` | `bool` | Không | Email đã xác thực chưa |
| `phoneConfirmed` | `bool` | Không | SĐT đã xác thực chưa |
| `twoFactorEnabled` | `bool` | Không | 2FA đang bật không |
| `status` | `AccountStatusEnum` | Không | Trạng thái tài khoản |
| `lastLoginAt` | `DateTime?` | Null nếu chưa login | Lần đăng nhập cuối (UTC) |
| `createdAt` | `DateTime` | Không | Thời điểm tạo (UTC) |
| `updatedAt` | `DateTime?` | Null nếu chưa update | Thời điểm cập nhật cuối (UTC) |
| `roles` | `string[]` | Không | Danh sách tên role (e.g. `["Admin"]`, `["Customer"]`) |
| `profile` | `AccountProfileDto?` | **Null nếu chưa gọi `PUT /api/auth/me/profile`** | Profile mở rộng |
| `staffProfile` | `StaffProfileDto?` | Null nếu không phải Staff hoặc chưa được Admin tạo | Staff profile, null nếu không phải Staff |

**StaffProfileDto shape:**
| Field | Type | Mô tả |
|---|---|---|
| `employeeCode` | `string` | Mã nhân viên |
| `department` | `string` | Phòng ban |
| `maxConcurrentTickets` | `int` | Số ticket tối đa xử lý đồng thời |
| `isAvailable` | `bool` | Trạng thái sẵn sàng nhận ticket |
| `notes` | `string` | Ghi chú |
| `displayAvatarUrl` | `string?` | Null nếu chưa có avatar | URL avatar đã resolve — **FE luôn dùng field này để render** |

### `AccountProfileDto`

Shape của `profile` trong `AccountDto` — `null` khi user chưa từng gọi `PUT /api/auth/me/profile`:

| Field | Type | Nullable | Mô tả |
|---|---|---|---|
| `accountId` | `Guid` | Không | ID tài khoản sở hữu profile |
| `avatarFileId` | `Guid?` | Null nếu chưa upload avatar | FileId từ FileStorageService |
| `externalAvatarUrl` | `string?` | Null nếu không dùng avatar ngoài | URL avatar từ Google hoặc nguồn ngoài |
| `avatarSource` | `AvatarSourceEnum` | Không | Nguồn avatar (xem enum) |
| `address` | `string?` | Null nếu chưa có | Địa chỉ |
| `birthDate` | `DateTime?` | Null nếu chưa có | Ngày sinh |
| `timeZone` | `string?` | Null nếu chưa có | Timezone (e.g. `Asia/Ho_Chi_Minh`) |
...
## Nhóm 1 — Xác thực (hầu hết Public — ngoại lệ: `POST /api/auth/logout` yêu cầu Bearer token + refresh token trong body)
...
| `data.expiresInSeconds` | `int` | Không | Thời gian hết hạn của resetToken (300 giây = 5 phút) |
...
**Rate limit / cooldown:** Endpoint có policy `AnonOtp` 5 request/phút theo IP. Ngoài ra có cooldown 60 giây giữa các lần resend — gọi quá sớm trả `429`. OTP reset password có TTL 5 phút. Response vẫn tránh tiết lộ email tồn tại hay không.
...
**Lưu ý — `profile` và `staffProfile` có thể `null`:** `AccountProfile` chỉ được tạo khi user tự gọi `PUT /api/auth/me/profile` lần đầu tiên. Trước đó `profile: null` là **hoàn toàn hợp lệ** — FE không được treat là lỗi. `staffProfile` tương tự — chỉ có khi Admin đã gọi `PUT /api/admin/staff/{id}/profile`.
...
**Lưu ý — `profile` và `staffProfile` có thể `null`:** `null` là trạng thái hợp lệ khi user chưa từng cập nhật profile — không phải lỗi eager load. FE phải handle `profile?.fullName ?? account.fullName` thay vì treat `null` là lỗi.
...
**Lưu ý — `profile` và `staffProfile` có thể `null`:** Xem lưu ý tại [GET /api/auth/me](#get-apiauthme) — cùng behavior.
...
**Response thành công `201`:** `isSuccess = true`, account đã tạo và email mời đã gửi.
...
```json
{
  ...
  "statusCode": 200,
  "data": 2,
  "message": "Admin đã thu hồi 2 session.",
  "listErrors": []
}
```

`data` là số session đã revoke.
...
**Response thành công `200`:** `CommonResponse<Guid>` — `data` là AccountId của staff vừa cập nhật.

**Lỗi thường gặp:**
- `400` — Dữ liệu không hợp lệ (maxConcurrentTickets ngoài 1–50, employeeCode quá 50 ký tự)
- `401` — Token không hợp lệ hoặc hết hạn
- `403` — Không có role Admin
- `404` — Không tìm thấy account
- `409` — `employeeCode` đã được sử dụng bởi staff khác
...
**Response thành công `200`:** `CommonResponse<Guid>` — `data` là AccountId của staff. FE dùng để reload `GET /api/staff/{id}/assignment-profile`.

**Lỗi thường gặp:**
- `400` — Dữ liệu không hợp lệ (skillLevel ngoài 1–5, skillCode quá 64 ký tự)
- `401` — Token không hợp lệ hoặc hết hạn
- `403` — Không có role Admin
- `404` — Không tìm thấy account
