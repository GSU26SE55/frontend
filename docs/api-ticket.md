...
**Mục đích:** Manager/Admin từ chối ticket khi không hợp lệ (spam, trùng lặp, ngoài scope dịch vụ). Chuyển trạng thái `Open → ClosedRejected` (từ bước Triage) **hoặc** `Escalated → ClosedRejected` (ticket đã chuyển cấp nhưng xác định ngoài scope). Lưu activity `Rejected` kèm `reason` vào timeline.

> **State machine (đã verify với `TicketStateMachine`):** hợp lệ từ **cả `Open` và `Escalated`** — xem `TransitionRuleProvider.cs` `#region Open Transitions` và `#region Escalated Transitions`.
...
- `401` — Chưa đăng nhập
- `403` — Không có role Manager/Admin, hoặc ticket không ở trạng thái `Open`/`Escalated`
- `404` — Không tìm thấy ticket
