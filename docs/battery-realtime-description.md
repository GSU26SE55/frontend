...
> 🚧 **`stats` — PLANNED, chưa wire ở FE (chờ BE deploy).** BE docs (Sprint Bonus NS-01/03/04 — #646/#648/#649) mô tả thêm event `stats` (rolling min/max dòng nạp/xả theo window `1h`/`today`) đẩy trên mọi scope. **Code FE hiện tại (`shared/lib/sse.ts`) mới chỉ đăng ký 3 event `reading`/`summary`/`ping`.** Payload `stats` xem §5.3bis (để tham chiếu khi implement). EventSource tự bỏ qua event lạ nên chưa nghe `stats` không gây lỗi — chỉ chưa có data min/max realtime; tạm backfill bằng REST `/aggregate` (§7.3).
...
> **Trạng thái:** BE docs mô tả event này (Sprint Bonus NS-01/03/04) nhưng **FE chưa wire** (xem ghi chú 🚧 ở §5). Giữ ở đây để làm chuẩn khi implement — **KHÔNG** phản ánh code FE hiện tại.
