...
- Wire `LiveTelemetryCard` vào admin `BatteryAssetDetailPage` Realtime section (seed REST `/latest`, fallback polling).
...
| `src/features/admin/pages/BatteryAssetDetailPage.tsx` | modify | Realtime section: dùng `useSensorStream(asset:{id})` + `<LiveTelemetryCard>`; `reading = stream.reading ?? latest ?? rt`; fallback polling. |
...
| GET | `/api/sensor-readings/{assetId}/latest` | (đã có) seed reading khi mở màn / sau reconnect → `CommonResponse<SensorReadingDto>` (8-field). |
...
```
**Render (admin `BatteryAssetDetailPage`):** `reading = stream.reading ?? latest(/latest) ?? rt(polling)`; badge: live→green pulse, connecting→amber, error/open-idle→gray.

## Edge Cases
- `Realtime:Enabled=false` (chỉ `ping`) → status "open-idle", hiển thị seed `/latest`, không lỗi/treo.
...
- Reconnect không replay (no `Last-Event-ID`) → REST `/latest` đắp gap.
...
- [ ] Admin `BatteryAssetDetailPage` Realtime hiển thị `<LiveTelemetryCard>` live ~5s, seed REST `/latest`, badge theo status.
- [ ] `Realtime:Enabled=false` → open-idle, hiển thị seed, không lỗi/treo.
...
## Steps
- [ ] B1 Enums: `shared/enums/telemetry.enum.ts`.
- [ ] B2 Types + endpoint: `shared/types/sensor-stream.types.ts` (`LiveReadingDto`, `SensorStreamState`) + `SENSOR_READINGS.STREAM`.
- [ ] B3 Schema + Wrapper: `shared/lib/sse.ts` (`liveReadingSchema` + `openSse`) — **verify route BE `SensorTelemetryStreamController.cs` trước**.
- [ ] B4 Hook: `shared/hooks/useSensorStream.ts` (nhánh asset reading, prefer primary, recreate cap).
- [ ] B5 Component: `shared/components/common/LiveTelemetryCard.tsx` + wire vào admin `BatteryAssetDetailPage`.
- [ ] B6 Gate: `tsc --noEmit` + `eslint --max-warnings=0` + `npm run build` → PASS.
...
- REST `/latest`·`/history`·`/aggregate` + `SensorReadingDto` đã wire ở `features/admin` (GH trước) — chỉ consume `/latest` để seed.
