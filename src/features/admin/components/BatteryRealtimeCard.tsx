...
export default function BatteryRealtimeCard({ assetId }: BatteryRealtimeCardProps) {
  const { data, isLoading } = useBatteryAssetRealtime(assetId);

  if (isLoading) return <div className="text-sm text-muted-foreground">Đang tải dữ liệu realtime...</div>;
  if (!data) return <div className="text-sm text-muted-foreground">Chưa có dữ liệu sensor</div>;
  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        ...
        {data.time && (
          <p className="text-xs text-muted-foreground">{new Date(data.time).toLocaleString('vi-VN')}</p>
        )}
      </CardHeader>
