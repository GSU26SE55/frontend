...
import { changeAccountStatusSchema, type ChangeAccountStatusFormValues } from '@/features/admin/schemas/admin-account.schema';
...
export default function ChangeAccountStatusDialog({ open, onClose, account }: Props) {
      ...
      <DialogContent className="sm:max-w-sm">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 py-2">
          <p className="text-sm text-muted-foreground">
            Tài khoản: <span className="font-medium text-foreground">{account.fullName}</span>
          </p>
            ...
            <Controller
              ...
              render={({ field }) => (
                <Select
                  ...
                  items={STATUS_OPTIONS.map(o => ({ value: String(o.value), label: o.label }))}
          ...
          <div className="space-y-1.5">
            <Label>Lý do</Label>
            <Input {...register('reason')} placeholder="Nhập lý do (tùy chọn)" />
          </div>
          <DialogFooter>
