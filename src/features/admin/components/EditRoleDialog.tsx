...
import { editRoleSchema, type EditRoleFormValues } from '@/features/admin/schemas/role.schema';
...
export default function EditRoleDialog({ open, onClose, role }: Props) {
  ...
  const onSubmit = async (data: EditRoleFormValues) => {
    try {
      await mutateAsync({ id: role.id, payload: { name: data.name, description: data.description || undefined } });
