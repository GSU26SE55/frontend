...
import { inviteAccountSchema, type InviteAccountFormValues } from '@/features/admin/schemas/admin-account.schema';
...
export default function InviteAccountDialog({ open, onClose }: Props) {
  ...
  const roles = Array.isArray(rolesData)
    ? rolesData
    : ((rolesData as { items?: unknown[] })?.items ?? []) as Array<{ id: string; name: string }>;

  const { mutateAsync, isPending } = useAdminInviteAccount();

  const {
    register, handleSubmit, setError, reset, control,
    formState: { errors },
  } = useForm<InviteAccountFormValues>({ resolver: zodResolver(inviteAccountSchema) });
    ...
    <Dialog open={open} onOpenChange={v => !v && handleClose()}>
      <DialogContent className="sm:max-w-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 py-2">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="invite-fullName">Họ và tên <span className="text-red-500">*</span></Label>
            <Input id="invite-fullName" {...register('fullName')} />
            {errors.fullName && <p className="text-xs text-red-500">{errors.fullName.message}</p>}
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="invite-phone">Số điện thoại</Label>
            <Input id="invite-phone" {...register('phoneNumber')} placeholder="0912345678" />
