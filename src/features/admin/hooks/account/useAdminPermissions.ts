...
import { adminPermissionsService } from "@/features/admin/services/account/admin-permissions.service";
import { KEY, QUERY_KEY } from "@/shared/utils/queryKeys";
import type { SetPermissionsPayload } from "@/features/admin/types/account/admin.types";

export const useAdminPermissionList = (module?: string) =>
  useQuery({
    queryKey: QUERY_KEY.admin.permissions.list(module),
    queryFn: () =>
      adminPermissionsService.getList(module).then((r) => r.data.data),
  });
...
export const useAdminRolePermissions = (roleId: string) =>
  useQuery({
    ...
    queryFn: () =>
      adminPermissionsService.getByRole(roleId).then((r) => r.data.data),
    enabled: !!roleId,
  });
