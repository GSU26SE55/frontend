...
import type { SiteFilterParams, SiteAssetsFilterParams, SiteCreatePayload, SiteUpdatePayload } from '@/shared/types/site.types';
...
export const useSiteAssets = (siteId: string, params?: SiteAssetsFilterParams) =>
  useQuery({
    ...
    queryFn:  () => adminSiteService.getAssets(siteId, params).then((r) => r.data.data),
    enabled:  !!siteId,
  });
...
export const useCreateSite = () => {
  ...
  return useMutation({
    mutationFn: (payload: SiteCreatePayload) => adminSiteService.create(payload),
    onSuccess:  () => { qc.invalidateQueries({ queryKey: [KEY.sites] }); },
  });
};
...
export const useDeleteSite = () => {
  ...
  return useMutation({
    ...
    onError: () => { toast.error('Xoá site thất bại'); },
  });
};
...
export const useRestoreSite = () => {
  ...
  return useMutation({
    ...
    onError: () => { toast.error('Khôi phục site thất bại'); },
  });
};
