...
export default function BatteryAssetDetailPage() {
  ...
  if (!asset) return <div className="p-6 text-muted-foreground">Không tìm thấy</div>;
  ...
  return (
    <div className="p-6 space-y-6 max-w-3xl">
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-2 gap-x-8 gap-y-2 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-medium">{asset.installDate.slice(0, 10)}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">Hết bảo hành</span>
            <span className="font-medium">{asset.warrantyEndDate?.slice(0, 10) ?? '—'}</span>
          </div>
          <div className="flex justify-between">
      ...
      <BatteryAssetForm open={editOpen} onOpenChange={setEditOpen} editData={asset} />
      <TransferOwnerDialog
        open={transferOpen}
