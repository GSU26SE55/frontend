...
import {
  RefreshCw, Plus, Users, Battery, MapPin, Settings,
      ...
      {trend && (
        <div className="flex items-center gap-1.5 text-xs" style={{ color: trendColor }}>
          {trend.dir === 'up'   ? <TrendingUp size={11} /> :
           trend.dir === 'down' ? <TrendingDown size={11} /> : null}
...
function AlertsChart() {
  const W = 560, H = 160, maxV = 28;
  const bw = 22, gap = 3, grpGap = 20;
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold">Cảnh báo theo tuần</h3>
        <div className="flex gap-3 text-[11px] text-muted-foreground">
          {[{ c: 'var(--p1)', l: 'Critical' }, { c: 'var(--p3)', l: 'Warning' }, { c: 'var(--border-strong)', l: 'Info' }].map((x) => (
            <span key={x.l} className="flex items-center gap-1">
              <span className="inline-block w-2 h-2 rounded-sm" style={{ background: x.c }} />
              {x.l}
            </span>
          ))}
        </div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H}>
        {[0.33, 0.67, 1].map((g, i) => (
          <line key={i} x1={0} x2={W} y1={H - 14 - g * (H - 26)} y2={H - 14 - g * (H - 26)}
            stroke="var(--border)" strokeDasharray="2 3" />
        ))}
        {ALERT_DATA.map((d, i) => {
          const bars = [{ v: d.crit, c: 'var(--p1)' }, { v: d.warn, c: 'var(--p3)' }, { v: d.info, c: 'var(--border-strong)' }];
          ...
          return (
            <g key={i}>
              {bars.map((b, j) => {
                ...
                return <rect key={j} x={x + j * (bw + gap)} y={H - 14 - bh} width={bw} height={bh} fill={b.c} rx={2} opacity={j === 2 ? 0.45 : 0.85} />;
              })}
              <text x={x + (grpW) / 2} y={H - 1} textAnchor="middle" fontSize="9.5" fill="var(--muted-foreground)"
                fontFamily="ui-monospace, monospace">{WEEKS[i]}</text>
            </g>
          );
        })}
      </svg>
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1440px]">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <KpiCard label="Total Sites"      value={totalSites}  sub="sites"           trend={{ dir: 'up',   value: `${activeSites}`, note: 'đang hoạt động' }} />
          <KpiCard label="Active Batteries" value={activeBatt}  sub={`/${totalBatt}`} trend={{ dir: 'up',   value: `+${activeBatt}`, note: 'online'          }} accent="var(--ok)" />
          <KpiCard label="Cần theo dõi"     value={totalBatt - activeBatt} sub="pin"  trend={{ dir: totalBatt - activeBatt > 0 ? 'down' : 'flat', value: `${totalBatt - activeBatt}`, note: 'inactive' }} accent={totalBatt - activeBatt > 0 ? 'var(--p1)' : undefined} />
        ...
        <Card className="lg:col-span-2 p-5">
          ...
          {isLoading ? (
            ...
            <div className="space-y-3">
              {sitesH.map((s) => (
                <button key={s.id} className="flex items-center gap-3 w-full text-left group" onClick={() => navigate(`/admin/sites/${s.id}`)}>
                  <div className="flex-1 min-w-0">
                    <p className="text-[12.5px] font-medium truncate group-hover:text-emerald-600 transition-colors">{s.name}</p>
                    <p className="text-[11px] text-muted-foreground">{s.batteryAssetCount} pin{s.capacityKw != null ? ` · ${s.capacityKw} kW` : ''}</p>
                  </div>
                  ...
                  <span className="text-[11.5px] font-semibold font-mono-num w-8 text-right shrink-0" style={{ color: hc(s.health) }}>{s.health}</span>
                </button>
              ))}
            </div>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card className="p-5">
          ...
          <ol className="space-y-4">
            {[
              { action: 'BATTERY.ASSIGN',   target: 'SN-B0042',  actor: 'Phan Quốc Hùng', note: 'Gán pin cho site ST-003',              ok: true,  dt: '21/05 · 14:32' },
              { action: 'SITE.UPDATE',      target: 'ST-001',    actor: 'Phan Quốc Hùng', note: 'Cập nhật capacity → 450 kW',           ok: true,  dt: '21/05 · 11:10' },
              { action: 'THRESHOLD.SET',    target: 'BT-LFP-01', actor: 'Admin',          note: 'Nhiệt độ max 47°C',                    ok: true,  dt: '20/05 · 09:48' },
              { action: 'USER.CREATE',      target: 'U-412',     actor: 'Admin',          note: 'Mời Staff mới — Nguyễn Văn B',         ok: true,  dt: '19/05 · 16:00' },
            ].map((item, i) => (
              <li key={i} className="flex gap-3">
                <span className="mt-[3px] w-2 h-2 rounded-full shrink-0" style={{ background: item.ok ? 'var(--ok)' : 'var(--p1)' }} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-baseline justify-between gap-2">
                    <span className="text-sm">
                      <span className="font-mono-num text-[11px] text-muted-foreground">{item.action}</span>
                      {' · '}<span className="font-medium">{item.target}</span>
                    </span>
                    <span className="font-mono-num text-[11px] text-muted-foreground shrink-0">{item.dt}</span>
                  </div>
                  <p className="text-[12.5px] text-muted-foreground">{item.note}</p>
                  <p className="text-[11px] text-muted-foreground/70 mt-0.5">{item.actor}</p>
                </div>
              </li>
            ))}
          </ol>
        ...
        <Card className="p-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { icon: <Users   size={16} />, title: 'Tạo user mới',       sub: 'Email + role + invite',     path: '/admin/accounts'       },
              { icon: <Battery size={16} />, title: 'Đăng ký battery',    sub: 'Asset + serial + site',     path: '/admin/battery-assets' },
              { icon: <MapPin  size={16} />, title: 'Tạo site',           sub: 'Vị trí, công suất',         path: '/admin/sites'          },
              { icon: <Settings size={16}/>, title: 'Cấu hình ngưỡng',   sub: 'Per battery type',          path: '/admin/battery-assets' },
            ].map((x, i) => (
              <button
                key={i}
              ...
              >
                <span className="text-muted-foreground group-hover:text-emerald-600 transition-colors">{x.icon}</span>
                <div>
                  <div className="text-[13px] font-semibold">{x.title}</div>
                  <div className="text-[11.5px] text-muted-foreground">{x.sub}</div>
                </div>
                <ExternalLink size={12} className="self-end text-muted-foreground/0 group-hover:text-muted-foreground/50 transition-colors" />
              </button>
            ))}
          </div>
        </Card>
