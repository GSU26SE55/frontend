...
import {
  Shield, Plus, Lock, CheckCircle, MoreHorizontal, Loader2,
  Edit2, ToggleLeft, Key, Trash2,
} from 'lucide-react';
...
import { useAdminRoleList, useAdminDeleteRole } from '@/features/admin/hooks/useAdminRoles';
...
export default function RolesPage() {
  ...
  const roles = Array.isArray(raw) ? raw : ((raw as { items?: unknown[] })?.items ?? []) as RoleDto[];
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <div className="text-[11px] text-muted-foreground font-mono">{role.normalizedName}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-44">
                        <DropdownMenuItem onClick={() => setDialog({ type: 'edit', role })}>
                          <Edit2 className="mr-2 size-4" /> Chỉnh sửa
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => setDialog({ type: 'status', role })}>
                          <ToggleLeft className="mr-2 size-4" /> Đổi trạng thái
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => setDialog({ type: 'perms', role })}>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {dialog.type === 'delete' && <>Bạn có chắc muốn xóa role <strong>{dialog.role.name}</strong>? Hành động này không thể hoàn tác.</>}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              variant="destructive"
              disabled={isDeleting}
              onClick={() => dialog.type === 'delete' && handleDelete(dialog.role)}
            >
