...
const MENU = [
  ...
  { key: '2fa',       label: 'Xác thực 2 lớp',        icon: ShieldCheck, desc: 'Bảo mật tài khoản nâng cao'         },
  { key: 'google',    label: 'Liên kết Google',        icon: Link2,       desc: 'Đăng nhập nhanh với Google'         },
  { key: 'history',   label: 'Lịch sử đăng nhập',     icon: History,     desc: 'Xem các phiên đăng nhập gần đây'    },
  { key: 'danger',    label: 'Danger Zone',            icon: Trash2,      desc: 'Xóa tài khoản & dữ liệu',  danger: true },
] as const;
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="px-6 py-5 border-b border-border bg-card shrink-0">
        ...
        <p className="text-sm text-muted-foreground mt-0.5">Quản lý thông tin và bảo mật tài khoản của bạn.</p>
      </div>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  'danger' in item && item.danger && !isActive && 'hover:bg-red-50 hover:text-red-600',
                  'danger' in item && item.danger && isActive && 'bg-red-50 text-red-600',
                )}
              >
