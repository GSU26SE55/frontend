    ...
    <Card>
      ...
      <CardContent className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <div>
          ...
          <p className={`text-2xl font-bold ${getHealthColor(data.healthScore)}`}>
            {data.healthScore}%
          </p>
          <p className={`text-sm font-medium ${getHealthColor(data.healthScore)}`}>
        ...
        <div>
          ...
          <p className="text-2xl font-bold text-green-600">{data.activeAssets}</p>
        </div>
        <div>
          <p className="text-sm text-muted-foreground">Cảnh báo đang mở</p>
          <p className={`text-2xl font-bold ${data.assetsWithActiveAlerts > 0 ? 'text-red-600' : ''}`}>
